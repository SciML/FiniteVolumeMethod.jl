...
@inline eval_condition_fnc(prob::AbstractFVMProblem, fidx, x, y, t,
    u) = eval_condition_fnc(prob.conditions, fidx, x, y, t, u)
@inline eval_source_fnc(prob::AbstractFVMProblem, x, y, t,
    u) = prob.source_function(x, y, t, u, prob.source_parameters)
...
@inline eval_flux_function(prob::FVMProblem, x, y, t, α, β,
    γ) = prob.flux_function(x, y, t, α, β, γ, prob.flux_parameters)

function FVMProblem(mesh::FVMGeometry, boundary_conditions::BoundaryConditions,
        internal_conditions::InternalConditions = InternalConditions();
        diffusion_function = nothing,
        diffusion_parameters = nothing,
...
@inline eval_flux_function(prob::SteadyFVMProblem, x, y, t, α, β,
    γ) = eval_flux_function(prob.problem, x, y, t, α, β, γ)
...
struct FVMSystem{N, FG, P, IC, FT, F, S, FF} <: AbstractFVMProblem
    ...
    source_functions::S
    flux_functions::FF
    function FVMSystem(mesh::FG, problems::P, initial_condition::IC, initial_time::FT,
            final_time::FT, conditions, num_fncs, functions::F, source_functions::S,
            flux_functions::FF) where {FG <: FVMGeometry, P, IC, FT, F, S, FF}
        ...
        @assert all(
            i -> problems[i].conditions.neumann_edges == conditions[i].neumann_edges, 1:length(problems)) "The Neumann edges in the `i`th `SimpleConditions` must match those from the `i`th problem."
        @assert all(
            i -> problems[i].conditions.constrained_edges ==
                 conditions[i].constrained_edges,
            1:length(problems)) "The constrained edges in the `i`th `SimpleConditions` must match those from the `i`th problem."
        @assert all(
            i -> problems[i].conditions.dirichlet_nodes == conditions[i].dirichlet_nodes,
            1:length(problems)) "The Dirichlet nodes in the `i`th `SimpleConditions` must match those from the `i`th problem."
...
function Base.show(io::IO, ::MIME"text/plain", prob::FVMSystem{N}) where {N}
    print(io, "FVMSystem with $N equations and time span ($(prob.initial_time), $(prob.final_time))")
end
...
@inline get_constrained_fidx(
    prob::FVMSystem, i, j, var) = get_constrained_edges(get_conditions(prob, var))[(
    i, j)]
@inline eval_condition_fnc(prob::FVMSystem, fidx, var, x, y, t,
    u) = eval_fnc_in_het_tuple(prob.functions, map_fidx(prob, fidx, var), x, y, t, u)
@inline eval_source_fnc(prob::FVMSystem, var, x, y, t,
    u) = eval_fnc_in_het_tuple(prob.source_functions, var, x, y, t, u)
...
@inline eval_flux_function(prob::FVMSystem, x, y, t, α, β,
    γ) = eval_all_fncs_in_tuple(prob.flux_functions, x, y, t, α, β, γ)
