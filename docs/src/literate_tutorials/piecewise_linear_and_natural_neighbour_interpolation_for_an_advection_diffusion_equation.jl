...
for i in eachindex(sol)
    ax = Axis(fig[1, i], width = 400, height = 400,
        ...
        title = "t = $(sol.t[i])",
        titlealign = :left)
    tricontourf!(ax, tri, sol.u[i], levels = 0:0.00001:0.001,
...
@test_reference joinpath(@__DIR__,
    "../figures",
    "piecewise_linear_and_natural_neighbour_interpolation_for_an_advection_diffusion_equation.png") fig #src
...
for (i, j) in enumerate(t_idx) #src
    ...
    tricontourf!(ax, tri, _sol.u[j], levels = 0:0.00001:0.001,
...
@test_reference joinpath(@__DIR__,
    "../figures",
    "piecewise_linear_and_natural_neighbour_interpolation_for_an_advection_diffusion_equation_exact_comparisons.png") fig #src
...
for i in eachindex(sol)
    ax = Axis(fig[1, i], width = 400, height = 400,
        ...
        title = "t = $(sol.t[i])",
        titlealign = :left)
    tricontourf!(ax, _tri, interpolated_vals[:, :, i] |> vec, levels = 0:0.00001:0.001,
        extendhigh = :auto, extendlow = :auto, colormap = :matter)
    tightlimits!(ax)
    ylims!(ax, -10, 10)
end
...
@test_reference joinpath(@__DIR__,
    "../figures",
    "piecewise_linear_and_natural_neighbour_interpolation_for_an_advection_diffusion_equation_piecewise_linear_interpolation.png") fig #src
...
fig = Figure(fontsize = 38)
all_vals = (sibson_vals, triangle_vals, laplace_vals, sibson_1_vals,
    nearest_vals, farin_vals, hiyoshi_vals, pde_vals)
titles = ("(a): Sibson", "(b): Triangle", "(c): Laplace", "(d): Sibson-1",
...
for (i, (vals, title)) in enumerate(zip(all_vals, titles))
    ax2d = Axis(fig[1, i], xlabel = "x", ylabel = "y", width = 600,
        height = 600, title = title, titlealign = :left)
    ax3d = Axis3(fig[2, i], xlabel = "x", ylabel = "y", width = 600,
        height = 600, title = title, titlealign = :left)
    ...
    ylims!(ax3d, -4, 4)
    if vals ≠ pde_vals
        contourf!(ax2d, _x, _y, vals, colormap = :matter,
        ...
        tricontourf!(ax2d, tri, vals, colormap = :matter, levels = 0:0.001:0.1,
        ...
        mesh!(ax3d, hcat(x, y, vals), triangles, color = vals,
...
@test_reference joinpath(@__DIR__,
    "../figures",
    "piecewise_linear_and_natural_neighbour_interpolation_for_an_advection_diffusion_equation_natural_neighbour_interpolation.png") fig #src
